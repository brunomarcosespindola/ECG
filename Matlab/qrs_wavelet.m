...
[Ty,Tx]  = findpeaks( abs(a3_atenuado(pos_T)), t3(pos_T),'MinPeakDistance',0.5); %encontra os picos T
%% LOCALIZAR A POSICAO DOS PONTOS

t3_matrix_P=repmat(t3,length(Px),1);
P_matrix= repmat(Px',1,length(t3));
posicao_P=find(sum(t3_matrix_P==P_matrix)) ;
clear t3_matrix_P; clear P_matrix;

t3_matrix_Q=repmat(t3,length(Qx),1);
Q_matrix= repmat(Qx',1,length(t3));
posicao_Q=find(sum(t3_matrix_Q==Q_matrix)) ;
clear t3_matrix_Q; clear Q_matrix;

t3_matrix_R=repmat(t3,length(peak_x),1);
R_matrix= repmat(peak_x',1,length(t3));
posicao_R=find(sum(t3_matrix_R==R_matrix)) ;
clear t3_matrix_R; clear R_matrix;

t3_matrix_S=repmat(t3,length(Sx),1);
S_matrix= repmat(Sx',1,length(t3));
posicao_S=find(sum(t3_matrix_S==S_matrix)) ;
clear t3_matrix_S; clear S_matrix;

t3_matrix_T=repmat(t3,length(Tx),1);
T_matrix= repmat(Tx',1,length(t3));
...
plot(t3(posicao_T), a3_atenuado(posicao_T),'xr')% marca onda T
...
plot(t(posicao_P*8-ordem*2.5),y(posicao_P*8-ordem*2.5), 'xk');% marca onda P 
plot(t(posicao_Q*8-ordem*2.5),y(posicao_Q*8-ordem*2.5), 'vb');% marca onda Q
plot(t(posicao_R*8-ordem*2.5),y(posicao_R*8-ordem*2.5), 'x');% marca onda R
plot(t(posicao_S*8-ordem*2.5),y(posicao_S*8-ordem*2.5), 'vg');% marca onda S
plot(t(posicao_T*8-ordem*2.5),y(posicao_T*8-ordem*2.5), 'xr');% marca onda T
%xlim([0 10])
